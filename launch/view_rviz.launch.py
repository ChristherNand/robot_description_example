...
def generate_launch_description():
    ...
    robot_description_file = os.path.join(
        get_package_share_directory('ros_example_package'),
        'urdf',
        'robot.urdf'
    )
    ...
    robot_state_publisher = Node(
        ...
        output='screen',
        parameters=[{'robot_description': Command(['xacro ', robot_description_file])}]
    )
    ...
    spawn_entity_node = Node(
        ...
        arguments=['-entity', 'waver', '-file', Command(['xacro ', robot_description_file]), '-x', '1.15', '-y', '-0.8', '-z', '0.5'],
        output='screen'
    )
